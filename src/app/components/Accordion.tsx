        ...
        <div>
            {
                faqs.map((faq, index) => (
                    <div key={faq.id} className={`my-3 flex flex-wrap justify-start overflow-hidden border-solid border border-BORDER_GRAY border-opacity-75   ${collapsed && indexBlock === `collapse-${faq.id}` ? 'bg-BLACK text-WHITE' : 'bg-WHITE text-TEXT_GREY'}`}>
                        <label className=" grow px-5 py-4 font-bold text-lg " htmlFor={`collapse-${faq.id}`}>
                            <span
                        ...
                        <input
                            ...
                            className="peer mx-5 my-4 h-0 w-0 appearance-none rounded border text-slate-800 accent-slate-600 opacity-0" type="checkbox" name={`collapse-${faq.id}`} id={`collapse-${faq.id}`}
                        />
